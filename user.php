...
include('header.php');
$tr=0;
if($cnfHomeCacheTime!=""){include "cache.php";}


$strLink="";
$strLinkEnd="";
$strLinkUser="?user=";
if($cnfPermaLink==0){$strLinkUser="user.php/";
$strLink="user.php/";
$strLinkEnd="/";
...
$strTitle="";
//$sup=0;
//echo $_SESSION['enviadorecover']."--".$_SESSION['timeSec']."--".$_SESSION['hashSec']."--".$_SESSION['iduserx'];
/*if(isset($_GET["q"]))$q=strtolower(filter_var($_GET["q"], FILTER_SANITIZE_FULL_SPECIAL_CHARS));
if(isset($_GET["w"]))$w=strtolower(filter_var($_GET["w"], FILTER_SANITIZE_FULL_SPECIAL_CHARS));*/
...
unset($_SESSION['tempUser']);
  ...
  $_SESSION['tempUser']=1;
  //"http://blitzhive.com/user.php/blitz/"
  //die($cnfHome.$strLink.$w.$strLinkEnd);
...
if(isset($_GET["user"]))$user=strtolower(filter_var($_GET["user"], FILTER_SANITIZE_FULL_SPECIAL_CHARS));
else {
$user=substr(rawurldecode($_SERVER['REQUEST_URI']),$posVar+9);
if(substr($user,-1)=="/")$user=substr($user,0,-1);
...
(isset($_SESSION['tempUser'])||$_POST["password"]!="")&&$_POST["password1"]!=""&&$_POST["password2"]!=""
){
...
if($_POST["password1"]!=$_POST["password2"])
	...
	if(!isset($_SESSION['tempUser']))$oldPassword=sha1($_SESSION['iduserx'].$_POST["password"]);	
...
<title><?php echo $user." | ".$cnfTitle;?></title>
	<?php
}else{
		$strTitle.="<title>".$user." | ".$cnfTitle."</title></head><body>";
	$strTitle.="<h1>".$lngUserNot." :(</h1>";
...
<body>
<div class="box1">
<a href="<?php echo $cnfHome; ?>" alt="<?php echo $lngBackIndex;?>" title="<?php echo $lngBackIndex;?>" ><img class="logo"  src="<?php echo $cnfHome.$cnfLogo;?>"  /></a>
<div class="boxAlignVertical">
<h2 class='h1Vertical'><?php echo $cnfHeaderText;?></h2>
<?php
//session_start();
...
/*if($blogMode==0){
if(($cnfForumCacheTime!=""&&$cnfForumCacheTime!="0")||($cnfMessageCacheTime!=""&&$cnfMessageCacheTime!="0")||($cnfCatCacheTime!=""&&$cnfCatCacheTime!="0")!isset($_SESSION['iduserx']))$cache = new SimpleCachePhp(__FILE__,$cnfForumCacheTime);
}else {
if($cnfHomeCacheTime!=""&&$cnfHomeCacheTime!="0"!isset($_SESSION['iduserx']))$cache = new SimpleCachePhp(__FILE__,$cnfHomeCacheTime);
}	
*/
/*if($cnfHomeCacheTime!=""&&$cnfHomeCacheTime!="0"){
...
<article class="boxPost" >
  <header>
	<?php echo "<h2 id='h2Title' class='h2Gray'>".$user."</h2>";?>
	
	<p class='pSubLine'>
	<?php if(isset($intVotes))echo fUser($intVotes,$user);?>
	</p><br>
	
  </header>
  <section><?php echo $strTitle;?></section>
  <aside>
<br style="clear:both;"> 
<span>
