...
if (isset($_SESSION['iduserx']))
    $user = $_SESSION['iduserx'];
if (isset($_GET['q']))
    $q = filter_var($_GET['q'], FILTER_SANITIZE_SPECIAL_CHARS);
$u = "0";
if (isset($_POST['u']))
    $u = filter_var($_POST['u'], FILTER_SANITIZE_SPECIAL_CHARS); //tags
$d = "0";
if (isset($_POST['d']))
    $d = filter_var($_POST['d'], FILTER_SANITIZE_SPECIAL_CHARS); //block
$f = "0";
if (isset($_POST['f']))
    $f = filter_var($_POST['f'], FILTER_SANITIZE_SPECIAL_CHARS); //ping
$po = "0";
if (isset($_POST['po']))
...
if (isset($_POST['w']))
  {
	...
	$w = filter_var($_POST['w'], FILTER_SANITIZE_SPECIAL_CHARS); //title		
  }
...
if (isset($_POST['submitFile']))
        ...
        if ($_SESSION['iduserx'] != $cnfAdm)
          {
            if ($cnfModAnswerAll != "")
            ...
            else if ($cnfModAnswerLevel != "")
              {
                ...
                if ($_SESSION['level'] < $cnfModAnswerLevel)
                    $modToken = "1";
              }
          }
        //die("-->".$t);
    ...
    if ($q == 0)
    ...
    else if ($q == 2)
      {
        ...
        if ($ifBlogMOde === false)
            ...
            die(header("refresh:0;url=" . $strLink . utf8_decode($_GET['w']) . $strLinkEnd . "#" . $t));
      }
  }
else if ($q == 1)
  {
    ...
    if ($interMinuts < 15 && $xml->p[0]->u == $user)
      {
        $last15 = 1;
      }
  }
...
else if ($q == 4)
  {
    ...
    $interMinuts = $interval / 60;
    if (isset($_SESSION['allowdelete']) && $interMinuts < 15 || $_SESSION['iduserx'] == $cnfAdm || $_SESSION['mod'] == $secForum)
      {
        $t = intval($t);
        if ($t == 0)
          {
...
else if ($q == 6 && ($_SESSION['iduserx'] == $cnfAdm || $_SESSION['mod'] == $secFOrum))
  {
    $w = utf8_decode($w);
    if (!file_exists($w . ".xml"))
        die($lngTheMes . " " . $w . " " . $lngNotExist);
